...
| `RATE_LIMIT_RPM` | *(none)* | Global request rate limit in requests per minute across all callers. Applies to `POST /v1/messages`, `/v1/chat/completions`, `/v1/completions`, `/v1/messages/count_tokens`, `/v1/embeddings`, `/v1/messages/batches`, `/v1/models/pull`, `GET /v1/models`, and `GET /v1/models/:id`. Returns `429` with `retry-after` header when exceeded |
