...
PROXY_MAX_CONCURRENCY=<n>     (optional; maximum simultaneous in-flight Ollama inference requests; when the limit is reached new requests get 503 overloaded_error + Retry-After: 1 instead of hammering the GPU and causing VRAM OOM errors; applies to POST /v1/messages, /v1/chat/completions, /v1/completions; embeddings and token-counting are not gated; default no limit)
